...
internal class ObjectDetector(
    ...
    fun detect(inputStream: InputStream, ortEnv: OrtEnvironment, ortSession: OrtSession): Result {
        ...
        Log.e(MainActivity.TAG, "Exception caught when perform ObjectDetection ${ortSession.inputInfo}")
        ...
        inputTensor.use {
            ...
            output.use {
                ...
                Log.e(MainActivity.TAG, "Exception caught when perform ObjectDetection ${rawOutput}")
                //val boxOutput = (output?.get(1)?.value) as Array<FloatArray>
                val outputImageBitmap = byteArrayToBitmap(rawOutput)

                // Step 5: set output result
                var result = Result(outputImageBitmap, arrayOf( floatArrayOf()))
                return result
            }
        }
    }
